/**
 * Artisan Profile Screen - Requirements: 22.1-22.11, 69.1, 69.2
 */
            ...
            {user?.profileImage ? (
              ...
              <View style={[styles.avatarPlaceholder, { backgroundColor: 'rgba(255,255,255,0.3)' }]}>
                <Text style={styles.avatarInitial}>
                  {(user?.businessName || user?.contactName || 'A')[0].toUpperCase()}
                </Text>
              </View>
            )}
          </View>
          <Text style={styles.name}>{user?.businessName || user?.contactName || 'Artisan'}</Text>
          {user?.contactName && user?.businessName && (
...
const styles = StyleSheet.create({
  ...
  contactName: { fontSize: 14, color: 'rgba(255,255,255,0.85)', marginBottom: 2 },
  email: { fontSize: 13, color: 'rgba(255,255,255,0.8)', marginBottom: 6 },
  ratingRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  ratingText: { fontSize: 13, color: 'rgba(255,255,255,0.9)' },
